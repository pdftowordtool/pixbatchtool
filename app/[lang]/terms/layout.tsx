...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].terms;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/terms`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/terms`;
    }
  }
...
export default function TermsLangLayout({ children }: { children: React.ReactNode }) {
  return children;
}
