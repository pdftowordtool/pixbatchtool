...
import esDict from '@/dictionaries/es/resize-jpg.json';
import deDict from '@/dictionaries/de/resize-jpg.json';
import itDict from '@/dictionaries/it/resize-jpg.json';
...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].resizeJpg;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/resize-jpg`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/resize-jpg`;
    }
  }
  languages['x-default'] = `${baseUrl}/resize-jpg`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/resize-jpg` : `${baseUrl}/${lang}/resize-jpg`;
...
export default function ResizeJpgLangLayout({
  children,
  params,
}: {
