...
import esCropPngDict from '@/dictionaries/es/crop-png.json';
import deCropPngDict from '@/dictionaries/de/crop-png.json';
import itCropPngDict from '@/dictionaries/it/crop-png.json';
...
const cropDicts: Record<string, typeof enCropPngDict> = {
  en: enCropPngDict,
  es: esCropPngDict,
  de: deCropPngDict,
  it: itCropPngDict,
...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].cropPng;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/crop-png`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/crop-png`;
    }
  }
  languages['x-default'] = `${baseUrl}/crop-png`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/crop-png` : `${baseUrl}/${lang}/crop-png`;
...
export default function CropPngLangLayout({
  children,
  params,
}: {
