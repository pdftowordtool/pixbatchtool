...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].contact;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/contact`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/contact`;
    }
  }
...
export default function ContactLangLayout({ children }: { children: React.ReactNode }) {
  return children;
}
