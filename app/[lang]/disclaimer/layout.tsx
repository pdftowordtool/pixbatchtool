...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].disclaimer;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/disclaimer`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/disclaimer`;
    }
  }
  languages['x-default'] = `${baseUrl}/disclaimer`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/disclaimer` : `${baseUrl}/${lang}/disclaimer`;
...
export default function DisclaimerLangLayout({ children }: { children: React.ReactNode }) {
  return children;
}
