...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].pngConverter;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/png-converter`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/png-converter`;
    }
  }
  languages['x-default'] = `${baseUrl}/png-converter`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/png-converter` : `${baseUrl}/${lang}/png-converter`;
...
export default function PngConverterLangLayout({
  children,
  params,
}: {
