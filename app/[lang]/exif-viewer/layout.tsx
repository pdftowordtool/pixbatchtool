...
import enExifDict from '@/dictionaries/en/exif-viewer.json';
import esExifDict from '@/dictionaries/es/exif-viewer.json';
import deExifDict from '@/dictionaries/de/exif-viewer.json';
import itExifDict from '@/dictionaries/it/exif-viewer.json';
...
const exifDicts: Record<string, typeof enExifDict> = {
  en: enExifDict,
  es: esExifDict,
  de: deExifDict,
  it: itExifDict,
...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].exifViewer;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/exif-viewer`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/exif-viewer`;
    }
  }
  languages['x-default'] = `${baseUrl}/exif-viewer`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/exif-viewer` : `${baseUrl}/${lang}/exif-viewer`;
...
export default function ExifViewerLangLayout({
  children,
  params,
}: {
  ...
  const faqEntities = exifDicts[lang].faq.items.map((item, index) => ({
    '@type': 'Question',
