...
import esRotateFlipDict from '@/dictionaries/es/rotate-flip-image.json';
import deRotateFlipDict from '@/dictionaries/de/rotate-flip-image.json';
import itRotateFlipDict from '@/dictionaries/it/rotate-flip-image.json';
...
const rotateFlipDicts: Record<string, typeof enRotateFlipDict> = {
  en: enRotateFlipDict,
  es: esRotateFlipDict,
  de: deRotateFlipDict,
  it: itRotateFlipDict,
...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].rotateFlipImage;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/rotate-flip-image`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/rotate-flip-image`;
    }
  }
  languages['x-default'] = `${baseUrl}/rotate-flip-image`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/rotate-flip-image` : `${baseUrl}/${lang}/rotate-flip-image`;
...
export default function RotateFlipLangLayout({
  children,
  params,
}: {
  ...
  const faqEntities = rotateFlipDicts[lang].faq.items.map((item, index) => ({
    '@type': 'Question',
