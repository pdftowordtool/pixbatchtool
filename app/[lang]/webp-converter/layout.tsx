...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].webpConverter;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/webp-converter`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/webp-converter`;
    }
  }
  languages['x-default'] = `${baseUrl}/webp-converter`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/webp-converter` : `${baseUrl}/${lang}/webp-converter`;
...
export default function WebpConverterLangLayout({
  children,
  params,
}: {
