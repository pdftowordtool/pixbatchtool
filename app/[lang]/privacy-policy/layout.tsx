...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].privacyPolicy;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/privacy-policy`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/privacy-policy`;
    }
  }
  languages['x-default'] = `${baseUrl}/privacy-policy`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/privacy-policy` : `${baseUrl}/${lang}/privacy-policy`;
...
export default function PrivacyPolicyLangLayout({ children }: { children: React.ReactNode }) {
  return children;
}
