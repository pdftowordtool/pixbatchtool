...
import esRemoveExifDict from '@/dictionaries/es/remove-exif.json';
import deRemoveExifDict from '@/dictionaries/de/remove-exif.json';
import itRemoveExifDict from '@/dictionaries/it/remove-exif.json';
...
const removeExifDicts: Record<string, typeof enRemoveExifDict> = {
  en: enRemoveExifDict,
  es: esRemoveExifDict,
  de: deRemoveExifDict,
  it: itRemoveExifDict,
...
export async function generateMetadata({
  ...
  const meta = metaDicts[lang].removeExif;

  const languages: Record<string, string> = {};
  for (const locale of i18n.locales) {
    if (locale === 'en') {
      languages[locale] = `${baseUrl}/remove-exif`;
    } else {
      languages[locale] = `${baseUrl}/${locale}/remove-exif`;
    }
  }
  languages['x-default'] = `${baseUrl}/remove-exif`;

  const canonicalUrl = lang === 'en' ? `${baseUrl}/remove-exif` : `${baseUrl}/${lang}/remove-exif`;
...
export default function RemoveExifLangLayout({
  children,
  params,
}: {
  ...
  const faqEntities = removeExifDicts[lang].faq.items.map((item, index) => ({
    '@type': 'Question',
